...
\[ \mu_i \in \left [ g\left( \hat{\eta}_i^{(L)}\right),  g\left( \hat{\eta}_i^{(U)} \right)      \right]   \]

\item En utilisant la méthode Delta, on obtient un I.C qui est centré, en utilisant le même I.C mais avec $\widehat{\mathrm{Var}}(\hat{\eta}_i)$ calculé avec
\begin{align*}
