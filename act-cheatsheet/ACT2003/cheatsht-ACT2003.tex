...
Soit $\ell(\theta, \phi ; Y) = \ln f(y ; \theta, \phi)$ la log-vraisemblance. Alors,
...
\begin{enumerate}[label = (2.\arabic*)]
	\item $\hat{\bm{\beta}}^{(k)} = \hat{\bm{\beta}}^{(k-1)} \mathcal{I} \left \{ \ddot{\ell}(\hat{\bm{\beta}})^{(k-1)} \right \}^{-1} \dot{\ell}(\hat{\bm{\beta}})^{(k-1)}$
...
Cette méthode est la même que l'algorithme de Newton-Raphson, à l'exception qu'on remplace $\mathcal{I} \left \{ \ddot{\ell}(\hat{\bm{\beta}}) \right \}$ par $\esp{\ddot{\ell}(\hat{\bm{\beta}})}$ à l'étape (2.1)
