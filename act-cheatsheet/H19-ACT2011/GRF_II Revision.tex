...
Bien qu'on puisse exercer l'option américaine au moment qu'on veut, il \underline{peut} être optimal d'exercer avant l'échéance si
\begin{align*}
PV(div) > K (1 - (1+r_f)^{-(T-t)})
\end{align*}
ou si
\begin{align*}
PV(div) > P(K, T-t) + K(1 - (1+r_f)^{-(T-t)})
\end{align*}
...
\hspace{5cm}\=\kill
 ...
 $\frac{C(K_1, T) - C(K_2, T)}{K_2 - K_1} \geq \frac{C(K_2, T) - C(K_3, T)}{K_3 - K_2}$ \>  $\frac{P(K_1, T) - P(K_2, T)}{K_2 - K_1} \geq \frac{P(K_2, T) - P(K_3, T)}{K_3 - K_2}$ \\
\end{tabbing}
Si le prix d'exercice est \textit{Constant en valeur actualisé}\footnote{i.e. $K_t = K(1+r_f)^{T}$.}, alors, avec $t < T$
\begin{align*}
C(K_t, t) & \leq C(K_T, T) \\
P(K_t, t) & \leq P(K_T, T) \\
\end{align*}
