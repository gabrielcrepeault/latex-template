...
\subsection*{Notation et relation de covariance à savoir}
\subsubsection*{Nouvelle notation}
\begin{align*}
X_{iw}			& = \sum_{t=1}^{n} \frac{w_{it}}{w_{ i\sum}} X_{it} \text{ , avec } w_{i\sum} = \sum_{t=1}^{n} w_{it} \\
X_{ww}		& = \sum_{i=1}^{I} \frac{w_{i \sum}}{w_{\sum \sum}} X_{iw} \text{ , avec } w_{\sum \sum} = \sum_{i=1}^{I} w_{i \sum}
\end{align*}
